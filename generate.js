    ...
    return items.map((g, i) => {
        ...
        return `
                <div class="gallery-item ${slideClass(i, cols)}" style="transition-delay:${(i % cols) * 90}ms">
                    <div class="phone-frame hover-3d shadow-xl">
...
/* overflow-x:hidden solo en body "se propaga" al viewport en algunos navegadores pero no es
   fiable -en movil se seguia pudiendo arrastrar la pagina hacia el lado y ver hueco en blanco,
   comprobado con window.scrollTo-. Ponerlo tambien en html lo hace infalible. */
html { overflow-x: hidden; }
/* position:relative es necesario para que overflow-x:hidden recorte de verdad a los .blob
   -son position:absolute y sin un ancestro posicionado se colocan respecto a toda la ventana,
   provocando el scroll lateral de mas en movil-. */
body { position: relative; font-family: var(--font); background: var(--bg); color: var(--text); line-height: 1.6; overflow-x: hidden; transition: background 0.3s, color 0.3s; }
...
/* Un nombre de clase no puede empezar por un numero -.3d-hover era invalido y el navegador
   ignoraba la regla entera, asi que el efecto no hacia nada-. */
.hover-3d:hover { transform: scale(1.02) translateY(-10px); transition: transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
