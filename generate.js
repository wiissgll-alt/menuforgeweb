...
// Map real country codes for FlagCDN to ensure flags render on ALL devices (especially Windows)
const FLAGS = { es: 'es', en: 'gb', fr: 'fr', it: 'it', de: 'de', pt: 'pt', cn: 'cn', sa: 'sa' };

function renderLangSwitcher(lang) {
    const currentFlag = `https://flagcdn.com/w20/${FLAGS[lang]}.png`;
    
    let options = languages.map(l => `
        <a href="${langPath(l)}" class="lang-option ${l === lang ? 'active' : ''}">
            <img src="https://flagcdn.com/w20/${FLAGS[l]}.png" alt="${l}" width="20" height="15" loading="lazy">
    ...
    <div class="custom-dropdown" id="lang-dropdown">
        <button class="dropdown-toggle" type="button" aria-haspopup="true" aria-expanded="false" onclick="toggleDropdown(event)">
            <img src="${currentFlag}" alt="${lang}" width="20" height="15">
...
/* Menú desplegable de banderas (Custom Dropdown) */
...
/* Phone Frame & Glow (Corregido: Brillo detrás, no encima) */
...
.phone-wrapper { position: relative; width: max-content; margin: 0 auto; z-index: 2; }
.phone-frame { border: 8px solid #1a1a2e; border-radius: 36px; overflow: hidden; background: #000; position: relative; z-index: 3; }
.phone-frame img { border-radius: 28px; display: block; width: 100%; }
.shadow-xl { box-shadow: var(--shadow-xl); }

/* El halo mágico vive en su propio contexto detras del telefono */
...
/* Forzamos que las imágenes de la galería corten sobrantes y midan exactamente lo mismo de alto */
...
/* ========================================= */
/*  RESPONSIVE (Ajustes para que no rompa)   */
/* ========================================= */
...
@media (max-width: 600px) {
    ...
    /* Alineación estricta de la línea de tiempo para evitar que se corte por la izquierda */
    .timeline { padding-left: 5px; }
    .timeline-item { gap: 1.2rem; }
    .timeline-badge { width: 40px; height: 40px; font-size: 1rem; }
    .timeline-line { left: 24px; } /* Justo en el centro del badge de 40px (5px pad + 20px centro - 1px) */
